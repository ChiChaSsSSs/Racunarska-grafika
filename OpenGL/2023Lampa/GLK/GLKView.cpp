...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_LEFT)
		m_glRenderer.yRotation -= 10;
	else if (nChar == VK_RIGHT)
		m_glRenderer.yRotation += 10;
	else if (nChar == VK_UP)
	{
		if (m_glRenderer.xRotation > -45)
			m_glRenderer.xRotation -= 5;
	}
	else if (nChar == VK_DOWN)
	{
		if (m_glRenderer.xRotation < 5)
			m_glRenderer.xRotation += 5;
	}
	else if (nChar == 'P')
	{
		if (m_glRenderer.zoom < -20)
			m_glRenderer.zoom += 10;
	}
	else if (nChar == 'O')
	{
		if (m_glRenderer.zoom > -120)
			m_glRenderer.zoom -= 10;
	}
	else if (nChar == '1')
	{
		if (m_glRenderer.firstArmRot < 30)
			m_glRenderer.firstArmRot += 5;
	}
	else if (nChar == '2')
	{
		if (m_glRenderer.firstArmRot > -30)
	...
	{
		if (m_glRenderer.secondArmRot < 30)
			m_glRenderer.secondArmRot += 5;
	}
	else if (nChar == '4')
	{
		if (m_glRenderer.secondArmRot > -30)
